...
% Weight Creation
N = (1: size(Banks, 2))';
a = 0.00001;                    % CCA Parameters
b = 0.1;
W = N.^(-a) + b;                % Weights Vector Creation
...
for i = 1:size(Data, 3)                     % For Each Trial
    ...
    Rho = zeros(size(Banks, 2), numel(StimFreq));

    for j = 1:size(Banks, 2)            % For Each Filtering Bank

        % Bank Seperation
        [b, a] = butter(3, Banks(:, j) / (Fs/2), 'bandpass');   % Filter Design
        SB = filtfilt(b, a, Data(:, :, i));                     % Signal Bank Filteration
    ...
    R = zeros(size(StimFreq));

    % Apply Weights
    for s = 1:numel(StimFreq)
        R(s) = sum(W .* Rho(:, s).^2);
    end
